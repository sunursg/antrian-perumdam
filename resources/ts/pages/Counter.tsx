...
export default function Counter({ user, organization, logoUrl, apiBase = "/counter-api" }: CounterProps) {
    ...
    return (
        <div className="min-h-screen bg-pdam-highlight text-slate-800 font-sans selection:bg-pdam-cyan/30 overflow-hidden relative flex flex-col">
            {/* Ambient Background */}
            <div className="fixed inset-0 z-0 pointer-events-none">
                <div className="absolute inset-0 bg-linear-to-br from-pdam-primary/10 via-transparent to-pdam-emerald/10"></div>
                <div className="absolute top-[-20%] left-[-10%] w-[50%] h-[50%] rounded-full bg-pdam-cyan/20 blur-[150px]"></div>
                <div className="absolute bottom-[-20%] right-[-10%] w-[50%] h-[50%] rounded-full bg-pdam-primary/20 blur-[150px]"></div>
            </div>

            {/* Header */}
            <header className="relative z-10 border-b border-pdam-deep-blue/10 bg-pdam-deep-blue text-white h-20 shadow-xl">
                <div className="max-w-[1920px] mx-auto px-6 h-full flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        <img src={logoUrl} alt="Logo" className="h-10 w-auto" />
                        <div>
                            <h1 className="text-lg font-black leading-tight tracking-wider uppercase">
                                {organization?.name ?? "PERUMDAM Tirta Perwira"}
                            </h1>
                            <div className="flex items-center gap-2">
                                <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-white/10 text-pdam-cyan border border-white/20 uppercase tracking-widest">
                                    Operator Dashboard
                                </span>
                            </div>
                        </div>
                    ...
                    <div className="flex items-center gap-4">
                        <a
                            href="/display"
                            target="_blank"
                            className="hidden md:flex items-center gap-2 px-4 py-2 rounded-lg bg-white/10 border border-white/10 hover:bg-white/20 text-sm font-bold transition-all"
                        >
            ...
            {/* Main Content */}
            <main className="flex-1 relative z-10 p-6 lg:p-8 flex flex-col xl:flex-row gap-6 lg:gap-8 max-w-[1920px] mx-auto w-full overflow-hidden">

                {/* Left Side: Controls */}
                <div className="flex-1 flex flex-col gap-6">

                    {/* Main Display Card (Blue/Green Block Theme) */}
                    <div className="flex-1 min-h-[400px] relative rounded-3xl overflow-hidden border-4 border-pdam-deep-blue bg-white shadow-2xl flex flex-col group transition-all duration-500">

                        {/* Header: Loket Info */}
                        <div className="bg-pdam-deep-blue py-4 text-center shrink-0 border-b border-white/10">
                            <h3 className="text-pdam-gold font-black tracking-[0.2em] text-xl uppercase">
                                {selectedLoket ? selectedLoket.name : "SILAKAN PILIH LOKET"}
                            </h3>
                        </div>
                        ...
                        <div className={cn(
                            ...
                            selectedLoket ? "bg-pdam-emerald" : "bg-slate-100"
                        )}>
                            {/* Shine effect */}
                            <div className="absolute inset-0 bg-linear-to-tr from-transparent via-white/10 to-transparent skew-x-12 -translate-x-full group-hover:translate-x-full transition-transform duration-1000" />
                            ...
                            {selectedLoket ? (
                                <>
                                    ...
                                    >
                                        <span className="text-white/90 font-bold tracking-[0.3em] text-sm uppercase mb-4 opacity-80">
                                            Sedang Melayani
                                        </span>
                                        ...
                                        <AnimatePresence mode="wait">
                                            <motion.h2
                                                ...
                                                className="text-[10rem] lg:text-[14rem] leading-none font-black text-white tracking-tighter drop-shadow-xl font-sans"
                                            >
                                                {currentTicket}
                                            </motion.h2>
                                        </AnimatePresence>
                                ...
                                <div className="relative z-10 flex flex-col items-center text-slate-400">
                                    <Monitor className="w-20 h-20 mb-6 opacity-30" />
                                    <p className="text-2xl font-black uppercase tracking-widest">Pilih loket untuk memulai</p>
                                </div>
                            )}
                        </div>

                        {/* Footer: Service Info */}
                        <div className="bg-pdam-primary py-5 text-center shrink-0 border-t border-white/10">
                            <p className="text-white font-black text-2xl tracking-widest uppercase">
                                {selectedLoket?.service?.name ?? "-"}
                            </p>
                        </div>
                    </div>
                    ...
                    {selectedLoket && stats && (
                        <div className="grid grid-cols-3 gap-4">
                            <div className="rounded-2xl bg-white shadow-lg border-2 border-pdam-deep-blue p-5 flex items-center gap-4 hover:scale-[1.02] transition-transform">
                                <div className="p-3 rounded-xl bg-pdam-primary/10 text-pdam-primary shadow-inner">
                                    <Users className="w-6 h-6" />
                                </div>
                                <div>
                                    <p className="text-[10px] text-slate-500 font-black uppercase tracking-widest mb-1">Total</p>
                                    <p className="text-3xl font-black text-pdam-deep-blue">{stats.total}</p>
                                </div>
                            </div>
                            <div className="rounded-2xl bg-white shadow-lg border-2 border-pdam-deep-blue p-5 flex items-center gap-4 hover:scale-[1.02] transition-transform">
                                <div className="p-3 rounded-xl bg-pdam-gold/10 text-amber-600 shadow-inner">
                                    <Activity className="w-6 h-6" />
                                </div>
                                <div>
                                    <p className="text-[10px] text-slate-500 font-black uppercase tracking-widest mb-1">Menunggu</p>
                                    <p className="text-3xl font-black text-pdam-deep-blue">{stats.pending}</p>
                                </div>
                            </div>
                            <div className="rounded-2xl bg-white shadow-lg border-2 border-pdam-deep-blue p-5 flex items-center gap-4 hover:scale-[1.02] transition-transform">
                                <div className="p-3 rounded-xl bg-pdam-emerald/10 text-pdam-emerald shadow-inner">
                                    <Clock className="w-6 h-6" />
                                </div>
                                <div>
                                    <p className="text-[10px] text-slate-500 font-black uppercase tracking-widest mb-1">Selesai</p>
                                    <p className="text-3xl font-black text-pdam-deep-blue">{stats.completed}</p>
                                </div>
                            </div>
                        </div>
                    )}
                ...
                <div className="xl:w-80 flex flex-col gap-4 h-full">
                    <div className="p-4 rounded-t-2xl bg-pdam-dark/40 border border-white/5 border-b-0 backdrop-blur-sm">
                    ...
                    <div className="flex-1 rounded-b-2xl bg-pdam-dark/20 border border-white/5 overflow-hidden flex flex-col">
